    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            if 651 > event.x() > 0 and 451 > event.y() > 0:
                ...
                x = float(self.request_params['ll'].split(',')[0]) - float(self.request_params['spn'].split(',')[0]) \
                    * 2.8 / 2
                y = float(self.request_params['ll'].split(',')[1]) - float(self.request_params['spn'].split(',')[1]) \
                    * 1.1 / 2
